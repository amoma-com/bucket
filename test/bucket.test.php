...
require_once __DIR__ . '/../src/bucket.inc.php';
...
class TestOfBucketAutoload extends UnitTestCase
{
    ...
    private $_spl_autoload_functions;

    function setUp()
    {
        $this->_spl_autoload_functions = spl_autoload_functions();
        if ($this->_spl_autoload_functions) {
            foreach ($this->_spl_autoload_functions as $fn) {
    ...
    function test_undefined_class_triggers_autoload()
    {
        ...
        $this->expectException('RequireUndefinedClass');
        $bucket->create('TestAutoloadFailedClass');
    }
